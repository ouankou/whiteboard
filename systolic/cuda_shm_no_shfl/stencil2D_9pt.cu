...
namespace stencil2d_9pt {
		...
		T fc, T fn0, T fs0, T fw0, T fe0, T fn1, T fs1, T fw1, T fe1)
	{
        ...
        // Each thread reads DATA_CACHE_SIZE pixels and save to a shared array
        // In this case, it has 8 pixels to process 4 pixels.
        // Thus, it's a BLOCK_SIZE*8 array.
        __shared__ T sdata[BLOCK_SIZE*DATA_CACHE_SIZE];
...
#pragma unroll
			for (int s = 0; s < DATA_CACHE_SIZE; s++) {
				int _tidy = tidy + s;
				sdata[threadIdx.x + s*BLOCK_SIZE] = src[index];
        ...
        // Calculate the needed indices
        int index_w1 = threadIdx.x;
        int index_w0 = threadIdx.x + 1;
        if (index_w0 >= BLOCK_SIZE) index_w0 = threadIdx.x;
        int index_c = threadIdx.x + 2;
        if (index_c >= BLOCK_SIZE) index_c = threadIdx.x;
        int index_e0 = threadIdx.x + 3;
        if (index_e0 >= BLOCK_SIZE) index_e0 = threadIdx.x;
        int index_e1 = threadIdx.x + 4;
        if (index_e1 >= BLOCK_SIZE) index_e1 = threadIdx.x;

        // Required data are read from shared memory
		#pragma unroll
		for (int i = 0; i < PROCESS_DATA_COUNT; i++) {
            T sum = sdata[index_e1 + (i+2)*BLOCK_SIZE] * fe1;

            sum += sdata[index_e0 + (i+2)*BLOCK_SIZE] * fe0;

            sum += sdata[index_c + (i+0)*BLOCK_SIZE] * fn1;
            sum += sdata[index_c + (i+1)*BLOCK_SIZE] * fn0;
            sum += sdata[index_c + (i+2)*BLOCK_SIZE] * fc;
            sum += sdata[index_c + (i+3)*BLOCK_SIZE] * fs0;
            sum += sdata[index_c + (i+4)*BLOCK_SIZE] * fs1;

            sum += sdata[index_w0 + (i+2)*BLOCK_SIZE] * fw0;

            sum += sdata[index_w1 + (i+2)*BLOCK_SIZE] * fw1;
	...
	static float Test(int width, int height) {
		...
		sprintf(szPath, "../Lena_cuda_shfl_proc_%dx%d.raw", width, height);
		imgDst.SaveRaw(szPath);

		sprintf(szPath, "../Lena_cuda_shfl_proc_%dx%d.txt", width, height);
