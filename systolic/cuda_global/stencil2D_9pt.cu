...
namespace stencil2d_9pt {
		...
		T fc, T fn0, T fs0, T fw0, T fe0, T fn1, T fs1, T fw1, T fe1)
	{
        ...
        // Prepare an local array to store all involved indices
        int global_index[DATA_CACHE_SIZE][FILTER_WIDTH];
		...
		int tidx_0 = process_count + laneId - FILTER_WIDTH / 2;
        ...
        // Required data are read from global memory
		#pragma unroll
		for (int i = 0; i < PROCESS_DATA_COUNT; i++) {
            T sum = src[global_index[i+2][4]] * fe1;

            sum += src[global_index[i+2][3]] * fe0;

            sum += src[global_index[i+0][2]] * fn1;
            sum += src[global_index[i+1][2]] * fn0;
            sum += src[global_index[i+2][2]] * fc;
            sum += src[global_index[i+3][2]] * fs0;
            sum += src[global_index[i+4][2]] * fs1;

            sum += src[global_index[i+2][1]] * fw0;

            sum += src[global_index[i+2][0]] * fw1;

			data[i] = sum;
		}
        // Reset the base index to current pixel
        tidx = tidx_0;
