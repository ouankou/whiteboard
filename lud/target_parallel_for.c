...
#pragma omp target data map(tofrom: matrix[0:size*size])
    {
...
#pragma omp target teams distribute parallel for map(tofrom: matrix[k*size:size*size-k*size])
        for (row = k + 1; row < size; row++) {
            matrix[row*size+k] /= matrix[k*size+k];
        };
#pragma omp target teams distribute parallel for private(row) shared(matrix) map(tofrom: matrix[k*size:size*size-k*size])
        for (row = k + 1; row < size; row++) {
            int col = 0;
